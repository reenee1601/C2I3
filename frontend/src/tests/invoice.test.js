...
describe('Invoice Page', () => {
    ...
    test('renders table with correct rows and columns', async () => {
        ...
        const soaIdHeader = screen.getByText('INVOICE ID');
        const supplierHeader = screen.getByText('SUPPLIER');
        const issuedDateHeader = screen.getByText('ISSUED DATE');
        ...
        expect(issuedDateHeader).toBeInTheDocument();
    ...
    test('Filter query bar appear when filter button is clicked', async () => {
        ...
        await waitFor(
            () => {
              const invoiceQueryBarVisible = screen.getByTestId('invoice-querybar');
              expect(invoiceQueryBarVisible).toBeInTheDocument();
            },
            {
              timeout,
            }
    ...
    test('sorting by clicking table header (invoice ID)', async () => {
      ...
      const sortedInvoiceIds = screen.getAllByTestId('invoice-id-link');
      expect(sortedInvoiceIds[0]).toHaveTextContent('0002');
      ...
      const sortedInvoiceIdsDescending = screen.getAllByTestId('invoice-id-link');
      expect(sortedInvoiceIdsDescending[0]).toHaveTextContent('2550');
    });
