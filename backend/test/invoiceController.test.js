...
textractData should handle OCR failure and return 500 status.
...
textractData should handle missing file and return 500 status.
...
textractData should return 200 status and empty data for successful OCR but no extracted data.
*/

const { textractData, uploadData, getData } = require('../controllers/invoiceController');
...
//   test('textractData should perform OCR successfully and return data', async () => {
...
//       /* Add expected form and table data properties here based on your use cases */
...
test('textractData should perform OCR successfully and return data', async () => {
  ...
  test('textractData should handle missing file and return 500 status', async () => {
  ...
  test('textractData should handle OCR failure and return 500 status', async () => {
    // Mocking an error thrown by performOcr
    jest.spyOn(require('../utils/utils'), 'performOcr').mockRejectedValue(new Error('OCR Error'));
  ...
  test('textractData should return 200 status and data for successful OCR', async () => {
    const req = { file: { path: '/mocked/path/to/file.txt' } };
    const res = {
      status: jest.fn(() => res),
      json: jest.fn(),
  ...
  test('textractData should return 200 status and empty data for successful OCR but no extracted data', async () => {
    const req = { file: { path: '/mocked/path/to/file.txt' } };
    const res = {
      status: jest.fn(() => res),
      json: jest.fn(),
  ...
  test('uploadData should upload data to MongoDB and return 200 status', async () => {
    const req = { body: {'Invoice ID': 'INV-12345',
    'Issued Date': '2023-08-01',
    'Supplier ID': 'SUPP-98765',
    'GST': '7%',
    'Total Before GST': '1000.00',
  ...
  test('uploadData should handle MongoDB create failure and return 500 status', async () => {
    ...
    jest.spyOn(require('../models/invoiceModel'), 'create').mockRejectedValue(new Error('MongoDB Create Error'));
  ...
  test('getData should fetch data from MongoDB and return 200 status', async () => {
    require('../models/invoiceModel').find.mockResolvedValueOnce(mockInvoiceData);
  ...
  test('getData should fetch data from MongoDB and return 200 status with empty invoice data', async () => {
    ...
    require('../models/invoiceModel').find.mockResolvedValue(mockEmptyInvoiceData);
  ...
  test('getData should handle MongoDB find failure and return 500 status', async () => {
    require('../models/invoiceModel').find.mockRejectedValue(new Error('MongoDB Find Error'));
    ...
    expect(res.status).toHaveBeenCalledWith(500);
    expect(res.json).toHaveBeenCalledWith({ error: 'Error fetching SOA data.' });
    expect(res.status).toHaveBeenCalledTimes(1);
    expect(res.json).toHaveBeenCalledTimes(1);
  });
  ...
  test('getData should handle negative invoice data and return 200 status', async () => {
    ...
    require('../models/invoiceModel').find.mockResolvedValue(mockNegativeInvoiceData);
